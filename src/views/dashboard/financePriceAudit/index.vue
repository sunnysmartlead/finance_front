...
const setPriceBoardStateAgree = async (isAgree: boolean) => {
  ElMessageBox["confirm"]("确定执行该操作?", "提示", {
    ...
    let res: any
    if (isAgree) {
      res = await SetFinancePriceState(auditFlowId, isAgree, opinionDescription.value)
    } else {
      if (opinionDescription.value) {
        res = await SetFinancePriceState(auditFlowId, isAgree, opinionDescription.value, checkList.value)
      } else {
        ElMessage({
          type: "warning",
          message: "拒绝理由必填"
