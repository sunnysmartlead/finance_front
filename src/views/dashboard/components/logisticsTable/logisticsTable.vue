<template>
  <el-table :data="logisticsData" border :height="logisticsData.length > 12 ? 675 : 'auto'"  :summary-method="(val: any) => getSummaries(val, '单PCS总物流成本', 'perTotalLogisticsCost')"
    ...
    <el-table-column align="center"  prop="monthEndDemand" label="月底需求量" :formatter="formatThousandths">
      <template #default="{ row }" v-if="isEdit">
        <el-input-number controls-position="right" :min="0" v-model="row.monthEndDemand" />
      </template>
    </el-table-column>
    <el-table-column align="center"  prop="storageExpenses" label="仓储费用/元" :formatter="formatThousandths" >
      <template #default="{ row }" v-if="isEdit">
        <el-input-number controls-position="right" :min="0" v-model="row.storageExpenses" />
      </template>
    </el-table-column>
    <el-table-column align="center"  prop="perPackagingPrice" label="单PCS包装价格/元" :formatter="formatThousandths" >
      <template #default="{ row }" v-if="isEdit">
        <el-input-number controls-position="right" :min="0" v-model="row.perPackagingPrice" />
      </template>
    </el-table-column>
    <el-table-column align="center"  prop="perFreight" label="单PCS运输费" :formatter="formatThousandths" >
      <template #default="{ row }" v-if="isEdit">
        <el-input-number controls-position="right" :min="0" v-model="row.perFreight" />
      </template>
    </el-table-column>
    <el-table-column align="center"  prop="perTotalLogisticsCost" label="单PCS总物流成本" :formatter="formatThousandths" >
      <template #default="{ row }" v-if="isEdit">
