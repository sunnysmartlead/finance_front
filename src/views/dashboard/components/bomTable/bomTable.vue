<template>
  <el-table :data="bomData" border height="500">
    <el-table-column prop="superType" label="超级大种类" width="175">
      <template #default="{ row }">
        <el-input v-model="row.superType" v-if="isEdit" />
      </template>
    </el-table-column>
    <el-table-column prop="materialName" label="材料名称" width="175">
      <template #default="{ row }">
    ...
    <el-table-column prop="assemblyCount" width="175" label="装配数量">
      <template #default="{ row }" v-if="isEdit">
        <el-input-number controls-position="right" :min="0" v-model="row.assemblyCount" />
      </template>
    </el-table-column>
    <el-table-column prop="materialPrice" label="材料单价（原币）" width="175" :formatter="toFixedThree">
      <template #default="{ row }" v-if="isEdit">
    ...
    <el-table-column prop="exchangeRate" label="汇率" width="175" :formatter="toFixedThree">
      <template #default="{ row }">
        <el-input-number controls-position="right" :min="0" v-model="row.exchangeRate" v-if="isEdit" />
      </template>
    </el-table-column>
    <el-table-column prop="materialPriceCyn" label="材料单价（人民币）" width="175" :formatter="toFixedThree">
      <template #default="{ row }">
        <el-input-number controls-position="right" :min="0" v-model="row.materialPriceCyn" v-if="isEdit" />
      </template>
    </el-table-column>
    <el-table-column prop="totalMoneyCyn" label="合计金额（人民币）- bom成本" width="175" :formatter="toFixedThree">
      <template #default="{ row }">
    ...
    <el-table-column prop="materialCost" label="材料成本（含损耗）" width="175" :formatter="toFixedThree">
      <template #default="{ row }">
        <el-input-number controls-position="right" :min="0" v-model="row.materialCost" v-if="isEdit" />
      </template>
    </el-table-column>

    <el-table-column prop="inputCount" label="投入量" width="175">
      <template #default="{ row }">
        <el-input-number controls-position="right" :min="0" v-model="row.inputCount" v-if="isEdit" />
      </template>
    </el-table-column>
    <el-table-column prop="purchaseCount" label="采购量" width="175">
      <template #default="{ row }">
        <el-input-number controls-position="right" :min="0" v-model="row.purchaseCount" v-if="isEdit" />
      </template>
    </el-table-column>
    <el-table-column prop="moqShareCount" label="MOQ分摊成本" width="175" :formatter="toFixedThree">
      <template #default="{ row }">
    ...
    <el-table-column prop="availableInventory" label="可用库存" width="175">
      <template #default="{ row }">
