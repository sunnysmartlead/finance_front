          ...
          <div class="table-box">
            <el-table :data="tableData" style="width: 100%;" 
                ...
                <template #default="scope">
                  <div>
                    <el-select  :disabled="currentEditProcessIndex != scope.row.processIndex" 
                      v-model="scope.row.processIndex"
                      filterable remote reserve-keyword 
                      :remote-method="remoteMethodForprocessIndex"
                      @change="processIndexChange($event, scope.$index)" 
              ...
              <el-table-column label="工装名称"   :width="tableColumnWidth" align="center">
                <template #default="scope">
                  <div>
                    <el-select :disabled="currentEditProcessIndex != scope.row.processIndex" v-model="scope.row.workClothesName"
                      filterable remote reserve-keyword :remote-method="remoteMethodForworkClothesName"
                      @change="workClothesNameChange($event, scope.$index)" :loading="optionLoading">
                      <el-option v-for="item in workClothesNameOptions" :key="item.value" :label="item.label" :value="item.value" />
                    </el-select>
                  </div>
                </template>
              </el-table-column>
                  ...
                  <div>
                    <!-- <el-input v-model="scope.row.workClothesPrice" :disabled="currentEditProcessIndex != scope.row.processIndex"
                      ...
                      <el-input-number v-model="scope.row.workClothesPrice" 
                              :disabled="currentEditProcessIndex != scope.row.processIndex"
              ...
              <el-table-column label="工装供应商"   :width="tableColumnWidth" align="center">
                <template #default="scope">
                  <div>
                    <el-input v-model="scope.row.workClothesProvider" :disabled="currentEditProcessIndex != scope.row.processIndex"
              ...
              <el-table-column label="工装维护人"  :width="tableColumnWidth" align="center">
                <template #default="scope">
                  <div>
                    <el-input v-model="scope.row.workClothesManager" :disabled="currentEditProcessIndex != scope.row.processIndex"
                      placeholder="请输入工装维护人" />
                  </div>
                </template>
              </el-table-column>
...
const processNameChange = (value: any, dataIndex: any) => {
  ...
  const getprocessIndex = (keyWord: String) => {
    return [
      { label: "工装编号1" + keyWord, value: '11111' },
      { label: "工装编号2" + keyWord, value: '22222' },
      { label: "工装编号3" + keyWord, value: '33333' },
      { label: "工装编号4" + keyWord, value: '44444' }
    ]
  }
  //监听工装名称变化
