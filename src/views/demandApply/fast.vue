        ...
        <el-row :gutter="20">
          ...
          <el-col :span="24">
            <el-form-item label="标题:" prop="title">
              <el-input v-model="state.quoteForm.title" disabled>
      ...
      <el-card class="demand-apply__card">
        <el-row :gutter="20">
          <el-col :span="8">
            <el-form-item label="项目代码:" prop="projectCode">
              <el-select
                v-model="state.quoteForm.projectCode"
                remote-show-suffix
                reserve-keyword
                filterable
                placeholder="Select"
                disabled
                remote
                :remote-method="getProjectCodeOptions"
          ...
          <el-col :span="6">
            <el-form-item label="客户名称:" prop="customerName">
              <el-input
                v-model="state.quoteForm.customerName"
          ...
          <el-col :span="6">
            <el-form-item label="终端名称:" prop="terminalName">
              <el-input
                v-model="state.quoteForm.terminalName"
                placeholder="终端名称"
                disabled
              />
            </el-form-item>
          ...
          <el-col :span="6">
            <el-form-item label="终端类别:" prop="terminalNature">
              <el-select
                v-model="state.quoteForm.terminalNature"
                placeholder="终端类别"
                disabled
              >
                <el-option
        ...
        <el-row>
          <el-row :span="15">
            <el-form-item label="核价类型:" prop="updateFrequency">
              <el-select
                v-model="state.quoteForm.priceEvalType"
                placeholder="核价类型"
                disabled
              >
        ...
        <el-row v-if="state.quoteForm.priceEvalType == 'PriceEvalType_Quantity'">
          <el-col :span="6">
            <el-form-item label="是否包含样品核价:" prop="updateFrequency">
              <el-select
                v-model="state.quoteForm.isHasSample"
                placeholder="是否包含样品核价"
                disabled
              >
            ...
            <el-table :data="specimenData" border style="width: 600px">
              <el-table-column prop="name" label="样品阶段" width="250">
                <template #default="{ row }">
                  <el-select v-model="row.name" placeholder="样品阶段" disabled>
              ...
              <el-table-column prop="pcs" label="需求量(pcs)" width="250">
                <template #default="{ row }">
                  <el-input v-model="row.pcs" disabled />
                </template>
              </el-table-column>
        ...
        <el-card header="项目走量">
          <el-row justify="space-between" align="middle" style="margin: 10px 0px">
            ...
            <el-button type="primary" @click="addPCS" disabled v-havedone
              >新增车型</el-button
            >
          </el-row>
          <el-table :data="pcsTableData" show-summary :summary-method="getPcsTableDatSummaries" border>
            <el-table-column label="车厂" width="180" fixed="left">
              <template #default="{ row }">
                <el-input v-model="row.carFactory" disabled />
              </template>
            </el-table-column>
            <el-table-column label="车型" width="180" fixed="left">
              <template #default="{ row }">
                <el-input v-model="row.carModel" disabled />
              </template>
            </el-table-column>
            <el-table-column
            ...
            <el-row justify="space-between" align="middle">
              ...
              <div>
                ...
                <el-button
                  ...
                  v-havedone
                  v-if="!isDisabled || (right === '1' && Findex === 0)"
                  >一键同步</el-button
                >
              </div>
            </el-row>
            <el-table :data="item" style="width: 100%" border>
              ...
              <el-table-column label="子项目代码" width="180">
                <template #default="{ row }">
                  ...
                  >
                    <el-option
                      v-for="item in projectSubCodeOptions"
                      :key="item.subCode"
                      :label="item.subCode"
                      :value="item.subCode"
                    />
              ...
              <el-table-column label="产品名称" width="180">
                <template #default="{ row }">
                  <el-input v-model="row.product" disabled @change="productChange" />
                </template>
              </el-table-column>
              ...
              <el-table-column label="像素" width="180">
                <template #default="{ row }">
                  <el-input v-model="row.pixel" placeholder="像素" disabled />
                </template>
              </el-table-column>
              <el-table-column label="我司角色" width="180">
                <template #default="{ row }">
                  <el-select v-model="row.ourRole" placeholder="我司角色" disabled>
                    <el-option
          ...
          <el-form-item label="是否分梯度核价：">
            <el-select
              v-model="state.quoteForm.isHasGradient"
              placeholder="是否分梯度核价"
              disabled
            >
              <el-option :value="true" label="是" />
          ...
          <el-row justify="end" v-if="state.quoteForm.isHasGradient">
            <el-button
              type="primary"
              disabled
              @click="handleChangekvPricingData('add')"
              v-havedone
              >新增梯度</el-button
            >
          </el-row>
        ...
        <el-table :data="requireTableData" style="width: 100%; margin: 20px 0" border>
          ...
          <el-table-column label="客户年降率(%)">
            <template #default="{ row, $index }">
              <el-input-number v-model="row.annualDeclineRate" :disabled="!$index" :min="0" />
            </template>
          </el-table-column>
          <el-table-column label="年度返利要求(%)">
            <template #default="{ row }">
              <el-input-number v-model="row.annualRebateRequirements" :disabled="right === '1'" :min="0" />
            </template>
          </el-table-column>
          <el-table-column label="一次性折让率(%)">
            <template #default="{ row }">
              <el-input-number v-model="row.oneTimeDiscountRate" :disabled="right === '1'" :min="0" />
            </template>
          </el-table-column>
          <el-table-column label="年度佣金比例(%)">
            <template #default="{ row }">
              <el-input-number v-model="row.commissionRate" :disabled="right === '1'" :min="0" />
            </template>
          </el-table-column>
        </el-table>
        <el-form-item label="是否有NRE费用分摊至模组:" prop="isHasNre">
          <el-select v-model="state.quoteForm.isHasNre" placeholder="Select" disabled>
            <el-option :value="true" label="是" />
            <el-option :value="false" label="否" />
          </el-select>
        </el-form-item>
        ...
        <el-row :gutter="20" v-if="state.quoteForm.isHasNre">
          <el-col :span="6">
            <el-form-item label="模具费分摊:" prop="allocationOfMouldCost">
              <el-select
                v-model="state.quoteForm.allocationOfMouldCost"
                placeholder="Select"
                disabled
              >
                <el-option :value="true" label="是" />
          ...
          <el-col :span="6">
            <el-form-item label="治具费分摊:" prop="allocationOfFixtureCost">
              <el-select
                v-model="state.quoteForm.allocationOfFixtureCost"
                placeholder="Select"
                disabled
              >
                <el-option :value="true" label="是" />
          ...
          <el-col :span="6">
            <el-form-item label="工装分摊:" prop="frockCost">
              <el-select
                v-model="state.quoteForm.frockCost"
                placeholder="Select"
                disabled
              >
                <el-option :value="true" label="是" />
          ...
          <el-col :span="6">
            <el-form-item label="检具费用分摊:" prop="fixtureCost">
              <el-select
                v-model="state.quoteForm.fixtureCost"
                placeholder="Select"
                disabled
              >
                <el-option :value="true" label="是" />
          ...
          <el-col :span="6">
            <el-form-item label="实验费用分摊:" prop="experimentCost">
              <el-select
                v-model="state.quoteForm.experimentCost"
                placeholder="Select"
                disabled
              >
                <el-option :value="true" label="是" />
          ...
          <el-col :span="6">
            <el-form-item label="测试软件费分摊:" prop="testCost">
              <el-select
                v-model="state.quoteForm.testCost"
                placeholder="Select"
                disabled
              >
                <el-option :value="true" label="是" />
          ...
          <el-col :span="6">
            <el-form-item label="差旅费分摊:" prop="travelCost">
              <el-select
                v-model="state.quoteForm.travelCost"
                placeholder="Select"
                disabled
              >
                <el-option :value="true" label="是" />
        ...
        <el-col :span="6">
          <el-form-item label="落地工厂:" prop="landingFactory">
            <el-select
              v-model="state.quoteForm.landingFactory"
              placeholder="请选择"
              disabled
            >
              <el-option
      ...
      <el-card class="demand-apply__card">
        ...
        <el-table :data="productTableData" style="width: 100%" border>
          <el-table-column label="产品名称" width="180" fixed="left">
            <template #default="{ row }">
              <el-input v-model="row.name"  :disabled="right === '1'" />
            </template>
          </el-table-column>
          <el-table-column label="Sensor" width="650">
            <template #default="{ row, $index }">
              <el-input v-model="row.sensor" placeholder="品牌/型号"  :disabled="right === '1'">
                <template #prepend>
                  <el-select v-model="row.sensorTypeSelect"  :disabled="right === '1'">
                    <el-option
          ...
          <el-table-column label="Lens" width="650">
            <template #default="{ row, $index }">
              <el-input v-model="row.lens" placeholder="品牌/型号"  :disabled="right === '1'">
                <template #prepend>
                  <el-select v-model="row.lensTypeSelect"  :disabled="right === '1'">
                    <el-option
                      v-for="item in state.TypeSelectOptions"
                      :key="item.id"
                      :label="item.displayName"
                  ...
                  >
                    <template #append>
                      <el-select
                        ...
                        @change="(val) => SensorChange(val, row, 'lensExchangeRate')"
                        :disabled="right === '1'"
                      >
                        <el-option
                          v-for="item in state.ExchangeSelectOptions"
          ...
          <el-table-column label="ISP" width="650">
            <template #default="{ row, $index }">
              <el-input v-model="row.isp" placeholder="品牌/型号"  :disabled="right === '1'">
                <template #prepend>
                  <el-select v-model="row.ispTypeSelect"  :disabled="right === '1'">
                    <el-option
                      v-for="item in state.TypeSelectOptions"
                      :key="item.id"
                      :label="item.displayName"
                  ...
                  >
                    <template #append>
                      <el-select
                        ...
                        @change="(val) => SensorChange(val, row, 'ispExchangeRate')"
                        :disabled="right === '1'"
                      >
                        <el-option
                          v-for="item in state.ExchangeSelectOptions"
          ...
          <el-table-column label="串行芯片" width="650">
            <template #default="{ row, $index }">
              <el-input v-model="row.serialChip" placeholder="品牌/型号"  :disabled="right === '1'">
                <template #prepend>
                  <el-select v-model="row.serialChipTypeSelect"  :disabled="right === '1'">
                    <el-option
                      v-for="item in state.TypeSelectOptions"
                      :key="item.id"
                      :label="item.displayName"
          ...
          <el-table-column label="线缆" width="650">
            <template #default="{ row, $index }">
              <el-input v-model="row.cable" placeholder="品牌/型号"  :disabled="right === '1'">
                <template #prepend>
                  <el-select v-model="row.cableTypeSelect"  :disabled="right === '1'">
                    <el-option
                      v-for="item in state.TypeSelectOptions"
                      :key="item.id"
                      :label="item.displayName"
          ...
          <el-table-column label="其它" width="650">
            <template #default="{ row, $index }">
              <el-input v-model="row.other" placeholder="品牌/型号" :disabled="right === '1'">
                <template #prepend>
                  <el-select v-model="row.otherTypeSelect" :disabled="right === '1'">
                    <el-option
                      v-for="item in state.TypeSelectOptions"
                      :key="item.id"
                      :label="item.displayName"
          ...
          <el-table-column label="制程" width="180">
            <template #default="{ row }">
              <el-input v-model="row.manufactureProcess" :disabled="right === '1'" />
            </template>
          </el-table-column>
          <el-table-column label="安装位置" width="180">
            <template #default="{ row }">
              <el-input v-model="row.installationPosition" :disabled="right === '1'" />
            </template>
          </el-table-column>
        </el-table>
        <h6>客户指定/供应详情</h6>
        <el-button
          @click="customerTargetPriceTable"
          type="primary"
          style="margin: 20px 0"
          :disabled="right === '1'"
          >点击生成</el-button
        ...
        <el-table :data="specifyTableData" style="width: 100%" border>
          ...
          <el-table-column prop="currency" label="币别">
            <template #default="{ row }">
              <el-select v-model="row.currency"  :disabled="right === '1'">
                <el-option
                  v-for="item in state.ExchangeSelectOptions"
                  :key="item.id"
                  :label="item.exchangeRateKind"
        ...
        <el-table :data="customerTargetPrice" border style="width: 100%">
          ...
          <el-table-column prop="目标价" label="目标价">
            <template #default="{ row }">
              <el-input v-model="row.targetPrice"  :disabled="right === '1'" />
            </template>
          </el-table-column>
          <el-table-column prop="currency" label="报价币种">
            <template #default="{ row, $index }">
              <el-select
                ...
                @change="(val) => cableTypeSelectChange(val, row, $index)"
                :disabled="right === '1'"
              >
                <el-option
                  v-for="item in state.ExchangeSelectOptions"
        ...
        <el-row :gutter="20">
          <el-col :span="6">
            <el-form-item label="贸易方式:" prop="tradeMode">
              <el-select
                v-model="state.quoteForm.tradeMode"
                placeholder="Select"
                :disabled="right === '1'"
              >
                <el-option
          ...
          <el-col :span="6">
            <el-form-item label="销售类型:" prop="salesType">
              <el-select
                v-model="state.quoteForm.salesType"
                placeholder="Select"
                :disabled="right === '1'"
              >
                <el-option
          ...
          <el-col :span="6">
            <el-form-item label="付款方式:" prop="paymentMethod">
              <el-input v-model="state.quoteForm.paymentMethod" :disabled="right === '1'" />
            </el-form-item>
          </el-col>
          <!-- <el-col :span="6">
        ...
        <el-row :gutter="20">
          <el-col :span="6">
            <el-form-item label="运输方式:" prop="shippingType">
              <el-select
                v-model="state.quoteForm.shippingType"
                placeholder="Select"
                :disabled="right === '1'"
              >
                <el-option
          ...
          <el-col :span="6">
            <el-form-item label="包装方式:" prop="packagingType">
              <el-select
                v-model="state.quoteForm.packagingType"
                placeholder="Select"
                :disabled="right === '1'"
              >
                <el-option
          ...
          <el-col :span="6">
            <el-form-item label="项目经理:" prop="projectManager" :disabled="right === '1'">
          ...
          <el-col :span="8">
            <el-form-item label="交货地点:" prop="placeOfDelivery">
              <el-input
                type="textarea"
                :rows="10"
                v-model="state.quoteForm.placeOfDelivery"
                :disabled="right === '1'"
              />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="核价原因:" prop="reason">
              <el-input
                type="textarea"
                :rows="10"
                v-model="state.quoteForm.reason"
                :disabled="right === '1'"
              />
            </el-form-item>
          </el-col>
...
import { priceEvaluationStartSave, getExchangeRate, getPriceEvaluationStartData, GetQuoteVersion, getWorkflowOvered, priceEvaluationStart } from "./service"
...
const handleSubmitData = async (isSubmit: boolean) => {
  ...
  try {
    let res: any = {}
    if (isSubmit) {
      res = await priceEvaluationStart(params)
    } else {
