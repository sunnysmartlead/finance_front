<template>
  ...
  >
    ...
    <el-table-column align="center"  prop="wastageCost" label="损耗成本" :formatter="toFixedTwo" >
      <template #default="{ row }" v-if="isEdit">
        <el-input-number controls-position="right" :min="0" v-model="row.wastageCost" />
      </template>
    </el-table-column>
    <el-table-column align="center"  prop="moqShareCount" label="MOQ分摊成本" :formatter="toFixedTwo" >
      <template #default="{ row }" v-if="isEdit">
