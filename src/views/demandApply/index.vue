      ...
      <el-card class="demand-apply__card">
        <el-row :gutter="20">
          <el-col :span="8">
            <el-form-item label="项目代码:" prop="projectCode">
              <el-select v-model="state.quoteForm.projectCode" remote-show-suffix reserve-keyword filterable
                placeholder="Select" :disabled="notEdit" remote :remote-method="getProjectCodeOptions"
          ...
          <el-col :span="8">
            <el-form-item label="项目名称:" prop="projectName">
              <el-input v-model="state.quoteForm.projectName" placeholder="与PLM系统保持一致" :disabled="notEdit" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="项目版本号:">
              <el-input disabled v-model="state.quoteForm.quoteVersion" />
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="客户名称:" prop="customerName">
              <el-input v-model="state.quoteForm.customerName" @change="generateTitle" :disabled="notEdit" />
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="客户类别:" prop="customerNature">
              <el-select v-model="state.quoteForm.customerNature" placeholder="客户类别" :disabled="notEdit">
          ...
          <el-col :span="6">
            <el-form-item label="终端名称:" prop="terminalName">
              <el-input v-model="state.quoteForm.terminalName" placeholder="终端名称" :disabled="notEdit" />
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="终端类别:" prop="terminalNature">
              <el-select v-model="state.quoteForm.terminalNature" placeholder="终端类别" :disabled="notEdit">
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="6">
            <el-form-item label="sop时间:" prop="sopTime">
              <el-date-picker type="year" placeholder="Pick a year" v-model="state.quoteForm.sopTime" value-format="YYYY"
                :disabled="notEdit" @change="yearChange" :disabled-date="disabledDate" />
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="价格有效期:" prop="updateFrequency">
              <el-select v-model="state.quoteForm.updateFrequency" placeholder="价格有效期" :disabled="notEdit"
                @change="yearChange(0)">
          ...
          <el-col :span="6">
            <el-form-item label="项目周期:" prop="projectCycle">
              <el-input-number @mousewheel.native.prevent v-model="state.quoteForm.projectCycle" @change="yearChange"
                :min="0" :disabled="notEdit" :max="8" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
        ...
        <el-card header="项目走量">
          <el-row justify="space-between" align="middle" style="margin: 10px 0px">
            ...
            <el-button type="primary" @click="addPCS" :disabled="notEdit" v-havedone>新增车型</el-button>
          </el-row>
          <el-table :data="pcsTableData" show-summary :summary-method="getPcsTableDatSummaries" border>
            <el-table-column label="车厂" width="180" fixed="left">
              <template #default="{ row }">
                <el-input v-model="row.carFactory" :disabled="notEdit" />
              </template>
            </el-table-column>
            <el-table-column label="车型" width="180" fixed="left">
              <template #default="{ row }">
                <el-input v-model="row.carModel" :disabled="notEdit" />
              </template>
            </el-table-column>
          ...
          <el-form :inline="true" :model="formInline" class="demo-form-inline">
            ...
            <el-form-item label="系数K">
              <el-input type="number" min="0" v-model="state.quoteForm.kValue" placeholder="手工录入" :disabled="notEdit" />
            </el-form-item>
          </el-form>
          ...
          <div v-for="(item, Findex) in moduleTableDataV2" :key="Findex">
            <el-row justify="space-between" align="middle">
              ...
              <div>
                <el-button type="primary" @click="addProduct(Findex)" v-havedone v-if="!isDisabled"
                  :disabled="notEdit">新增模组</el-button>
            ...
            <el-table :data="item" style="width: 100%" border>
              ...
              <el-table-column label="客户零件号" width="180">
                <template #default="{ row }">
                  <el-input v-model="row.partNumber" placeholder="若没有，填写 /" :disabled="notEdit" />
                </template>
              </el-table-column>
              <el-table-column label="子项目代码" width="180">
                <template #default="{ row }">
                  <el-select v-model="row.code" placeholder="Select" :disabled="notEdit"
                    @change="(v) => changeCode(v, row)">
                    <el-option v-for="item in projectSubCodeOptions" :key="item.subCode" :label="item.subCode"
                      :value="item.subCode" />
                  </el-select>
                </template>
              </el-table-column>
              <el-table-column label="产品名称" width="180">
                <template #default="{ row }">
                  <el-input v-model="row.product" :disabled="notEdit" @change="productChange" />
                </template>
              </el-table-column>
              <el-table-column label="产品大类" width="180">
                <template #default="{ row }">
                  <el-select v-model="row.productType" placeholder="产品大类" :disabled="notEdit">
              ...
              <el-table-column label="像素" width="180">
                <template #default="{ row }">
                  <el-input v-model="row.pixel" placeholder="像素" :disabled="notEdit" />
                </template>
              </el-table-column>
              <el-table-column label="我司角色" width="180">
                <template #default="{ row }">
                  <el-select v-model="row.ourRole" placeholder="我司角色" :disabled="notEdit">
              ...
              <el-table-column label="市场份额" width="180">
                <template #default="{ row }">
                  <el-input v-model="row.marketShare" oninput="value=value.replace(/[^0-9.]/g,'')" :disabled="notEdit"
                    type="number">
          ...
          <el-form-item label="是否分梯度核价：">
            <el-select v-model="state.quoteForm.isHasGradient" placeholder="是否分梯度核价" :disabled="notEdit">
              <el-option :value="true" label="是" />
              <el-option :value="false" label="否" />
            </el-select>
          </el-form-item>
          <el-row justify="end" v-if="state.quoteForm.isHasGradient">
            <el-button type="primary" :disabled="notEdit" @click="handleChangekvPricingData('add')"
          ...
          <div v-for="(item, Findex) in gradientModelTable" :key="Findex">
            <p class="mx-1" size="large">
              {{ `${item.kv} ${state.quoteForm.updateFrequency == updateFrequency.HalfYear ? "(K/HY)" : "(K/Y)"}` }}
            </p>
        ...
        <div v-if="state.quoteForm.isHasNre">
          ...
          <el-table :data="shareCountTable">
            ...
            <el-table-column prop="yearCount" label="分摊年数" width="250">
              <template #default="{ row, $index }">
                <el-select v-model="row.yearCount" placeholder="Select" :disabled="notEdit"
        ...
        <el-row :gutter="20" v-if="state.quoteForm.isHasNre">
          <el-col :span="6">
            <el-form-item label="模具费分摊:" prop="allocationOfMouldCost">
              <el-select v-model="state.quoteForm.allocationOfMouldCost" placeholder="Select" :disabled="notEdit">
                <el-option :value="true" label="是" />
                <el-option :value="false" label="否" />
              </el-select>
            </el-form-item>
          ...
          <el-col :span="6">
            <el-form-item label="治具费分摊:" prop="allocationOfFixtureCost">
              <el-select v-model="state.quoteForm.allocationOfFixtureCost" placeholder="Select" :disabled="notEdit">
                <el-option :value="true" label="是" />
                <el-option :value="false" label="否" />
              </el-select>
            </el-form-item>
          ...
          <el-col :span="6">
            <el-form-item label="工装分摊:" prop="frockCost">
              <el-select v-model="state.quoteForm.frockCost" placeholder="Select" :disabled="notEdit">
                <el-option :value="true" label="是" />
                <el-option :value="false" label="否" />
              </el-select>
            </el-form-item>
          ...
          <el-col :span="6">
            <el-form-item label="专用设备费分摊:" prop="allocationOfEquipmentCost">
              <el-select v-model="state.quoteForm.allocationOfEquipmentCost" placeholder="Select" :disabled="notEdit">
                <el-option :value="true" label="是" />
                <el-option :value="false" label="否" />
              </el-select>
            </el-form-item>
          ...
          <el-col :span="6">
            <el-form-item label="检具费用分摊:" prop="fixtureCost">
              <el-select v-model="state.quoteForm.fixtureCost" placeholder="Select" :disabled="notEdit">
                <el-option :value="true" label="是" />
                <el-option :value="false" label="否" />
              </el-select>
            </el-form-item>
          ...
          <el-col :span="6">
            <el-form-item label="实验费用分摊:" prop="experimentCost">
              <el-select v-model="state.quoteForm.experimentCost" placeholder="Select" :disabled="notEdit">
                <el-option :value="true" label="是" />
                <el-option :value="false" label="否" />
              </el-select>
            </el-form-item>
          ...
          <el-col :span="6">
            <el-form-item label="测试软件费分摊:" prop="testCost">
              <el-select v-model="state.quoteForm.testCost" placeholder="Select" :disabled="notEdit">
                <el-option :value="true" label="是" />
                <el-option :value="false" label="否" />
              </el-select>
            </el-form-item>
          ...
          <el-col :span="6">
            <el-form-item label="差旅费分摊:" prop="travelCost">
              <el-select v-model="state.quoteForm.travelCost" placeholder="Select" :disabled="notEdit">
                <el-option :value="true" label="是" />
                <el-option :value="false" label="否" />
              </el-select>
            </el-form-item>
          ...
          <el-col :span="6">
            <el-form-item label="其他费用分摊:" prop="otherCost">
              <el-select v-model="state.quoteForm.otherCost" placeholder="Select" :disabled="notEdit">
                <el-option :value="true" label="是" />
                <el-option :value="false" label="否" />
              </el-select>
            </el-form-item>
        ...
        <el-col :span="6">
          <el-form-item label="落地工厂:" prop="landingFactory">
            <el-select v-model="state.quoteForm.landingFactory" placeholder="请选择" :disabled="notEdit">
      ...
      <el-card class="demand-apply__card">
        ...
        <el-table :data="productTableData" style="width: 100%" border>
          ...
          <el-table-column label="Sensor" width="650">
            <template #default="{ row, $index }">
              <el-input v-model="row.sensor" placeholder="品牌/型号" :disabled="!canDo || right === '1'">
                <template #prepend>
                  <el-select v-model="row.sensorTypeSelect" :disabled="!canDo || right === '1'">
                ...
                <template #append>
                  <el-input v-model="row.sensorPrice" placeholder="单价" oninput="value=value.replace(/[^0-9.]/g,'')"
                    ...
                    <template #append>
                      <el-select v-model="row.sensorCurrency"
                        @change="(val) => SensorChange(val, row, 'sensorExchangeRate')"
                        :disabled="!canDo || right === '1'">
                  ...
                  <el-input v-model="row.sensorExchangeRate" placeholder="汇率:" :disabled="!canDo || right === '1'"
                    style="width: 100px" type="number" oninput="value=value.replace(/[^0-9.]/g,'')" />
                </template>
              </el-input>
            </template>
          </el-table-column>
          <el-table-column label="Lens" width="650">
            <template #default="{ row, $index }">
              <el-input v-model="row.lens" placeholder="品牌/型号" :disabled="!canDo || right === '1'">
                <template #prepend>
                ...
                <template #append>
                  ...
                  <el-input v-model="row.lensExchangeRate" placeholder="汇率:" :disabled="!canDo || right === '1'"
                    style="width: 100px" type="number" oninput="value=value.replace(/[^0-9.]/g,'')" />
                </template>
              </el-input>
            </template>
          ...
          <el-table-column label="ISP" width="650">
            <template #default="{ row, $index }">
              <el-input v-model="row.isp" placeholder="品牌/型号" :disabled="!canDo || right === '1'">
                <template #prepend>
                  <el-select v-model="row.ispTypeSelect" :disabled="!canDo || right === '1'">
                    <el-option v-for="item in state.TypeSelectOptions" :key="item.id" :label="item.displayName"
                      :value="item.id" />
                  </el-select>
                </template>
                <template #append>
                  ...
                  <el-input v-model="row.ispExchangeRate" placeholder="汇率:" :disabled="!canDo || right === '1'"
                    style="width: 100px" type="number" oninput="value=value.replace(/[^0-9.]/g,'')" />
                </template>
              </el-input>
            </template>
          ...
          <el-table-column label="串行芯片" width="650">
            <template #default="{ row, $index }">
              <el-input v-model="row.serialChip" placeholder="品牌/型号" :disabled="!canDo || right === '1'">
                <template #prepend>
                  <el-select v-model="row.serialChipTypeSelect" :disabled="!canDo || right === '1'">
                    <el-option v-for="item in state.TypeSelectOptions" :key="item.id" :label="item.displayName"
                      :value="item.id" />
                  </el-select>
                </template>
                <template #append>
                  <el-input v-model="row.serialChipPrice" placeholder="单价" oninput="value=value.replace(/[^0-9.]/g,'')"
                    ...
                    <template #append>
                      <el-select v-model="row.serialChipCurrency"
                        @change="(val) => SensorChange(val, row, 'serialChipExchangeRate')"
                        :disabled="!canDo || right === '1'">
                        <el-option v-for="item in state.ExchangeSelectOptions" :key="item.id"
                          :label="item.exchangeRateKind" :value="item.id" />
                      </el-select>
                  ...
                  <el-input v-model="row.serialChipExchangeRate" placeholder="汇率:" :disabled="!canDo || right === '1'"
                    style="width: 100px" type="number" oninput="value=value.replace(/[^0-9.]/g,'')" />
                </template>
              </el-input>
            </template>
          ...
          <el-table-column label="线缆" width="650">
            <template #default="{ row, $index }">
              <el-input v-model="row.cable" placeholder="品牌/型号" :disabled="!canDo || right === '1'">
                <template #prepend>
                  <el-select v-model="row.cableTypeSelect" :disabled="!canDo || right === '1'">
                    <el-option v-for="item in state.TypeSelectOptions" :key="item.id" :label="item.displayName"
                      :value="item.id" />
                  </el-select>
                </template>
                <template #append>
                  <el-input v-model="row.cablePrice" placeholder="单价" oninput="value=value.replace(/[^0-9.]/g,'')"
                    ...
                    <template #append>
                      <el-select v-model="row.cableCurrency"
                        @change="(val) => SensorChange(val, row, 'cableExchangeRate')"
                        :disabled="!canDo || right === '1'">
                        <el-option v-for="item in state.ExchangeSelectOptions" :key="item.id"
                          :label="item.exchangeRateKind" :value="item.id" />
                      </el-select>
                  ...
                  <el-input v-model="row.cableExchangeRate" placeholder="汇率:" :disabled="!canDo || right === '1'"
                    style="width: 100px" type="number" oninput="value=value.replace(/[^0-9.]/g,'')" />
                </template>
              </el-input>
            </template>
          ...
          <el-table-column label="其它" width="650">
            <template #default="{ row, $index }">
              <el-input v-model="row.other" placeholder="品牌/型号" :disabled="!canDo || right === '1'">
                <template #prepend>
                  <el-select v-model="row.otherTypeSelect" :disabled="!canDo || right === '1'">
                    <el-option v-for="item in state.TypeSelectOptions" :key="item.id" :label="item.displayName"
                      :value="item.id" />
                  </el-select>
                </template>
                <template #append>
                  <el-input v-model="row.otherPrice" placeholder="单价" oninput="value=value.replace(/[^0-9.]/g,'')"
                    ...
                    <template #append>
                      <el-select v-model="row.otherCurrency"
                        @change="(val) => SensorChange(val, row, 'otherExchangeRate')"
                        :disabled="!canDo || right === '1'">
                        <el-option v-for="item in state.ExchangeSelectOptions" :key="item.id"
                          :label="item.exchangeRateKind" :value="item.id" />
                      </el-select>
                  ...
                  <el-input v-model="row.otherExchangeRate" placeholder="汇率:" :disabled="!canDo || right === '1'"
                    style="width: 100px" type="number" oninput="value=value.replace(/[^0-9.]/g,'')" />
                </template>
              </el-input>
            </template>
          ...
          <el-table-column label="制程" width="180">
            <template #default="{ row }">
              <el-input v-model="row.manufactureProcess" :disabled="!canDo || right === '1'" />
            </template>
          </el-table-column>
          <el-table-column label="安装位置" width="180">
            <template #default="{ row }">
              <el-input v-model="row.installationPosition" :disabled="!canDo || right === '1'" />
            </template>
          </el-table-column>
        </el-table>
        <h6>客户指定/供应详情</h6>
        <el-button @click="customerTargetPriceTable" type="primary" style="margin: 20px 0"
          :disabled="!canDo || right === '1'">点击生成</el-button>
        ...
        <el-table :data="customerTargetPrice" border style="width: 100%">
          ...
          <el-table-column prop="目标价" label="目标价">
            <template #default="{ row }">
              <el-input v-model="row.targetPrice" :disabled="!canDo || right === '1'" />
            </template>
          </el-table-column>
          ...
          <el-table-column prop="exchangeRate" label="汇率">
            <template #default="{ row, $index }">
              <el-input v-model="row.exchangeRate" :disabled="!canDo || right === '1'"
                @change="(val) => changeExchangRate(val, $index)" type="number"
        ...
        <el-row :gutter="20">
          <el-col :span="6">
            <el-form-item label="贸易方式:" prop="tradeMode">
              <el-select v-model="state.quoteForm.tradeMode" placeholder="Select" :disabled="!canDo || right === '1'">
          ...
          <el-col :span="6">
            <el-form-item label="销售类型:" prop="salesType">
              <el-select v-model="state.quoteForm.salesType" placeholder="Select" :disabled="!canDo || right === '1'">
          ...
          <el-col :span="6">
            <el-form-item label="付款方式:" prop="paymentMethod">
              <el-input v-model="state.quoteForm.paymentMethod" :disabled="!canDo || right === '1'" />
            </el-form-item>
          </el-col>
          <!-- <el-col :span="6">
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="6">
            <el-form-item label="包装方式:" prop="packagingType">
              <el-select v-model="state.quoteForm.packagingType" placeholder="Select" :disabled="!canDo || right === '1'">
          ...
          <el-col :span="6">
            <el-form-item label="项目经理:" prop="projectManager" :disabled="!canDo || right === '1'">
          ...
          <el-col :span="8">
            <el-form-item label="核价原因:" prop="reason">
              <el-input type="textarea" :rows="10" v-model="state.quoteForm.reason" :disabled="!canDo || right === '1'" />
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item prop="sorFile">
              <el-upload v-model:file-list="fileList" :action="$baseUrl + 'api/services/app/FileCommonService/UploadFile'"
                ...
                :on-progress="handleGetUploadProgress" show-file-list :disabled="!canDo || right === '1'">
...
const getProjectCodeOptions = async (filter?: any, key?: string) => {
...
const save = debounce(async (formEl: FormInstance | undefined, isSubmit: boolean) => {
...
const handleSubmitData = async (isSubmit: boolean) => {
  ...
  // return console.log(state.opinion, "state.opinion111")
  const params = {
    ...quoteForm,
    // comment,
      ...
      if (moduleTableTotalData?.length && kvList?.length && isFirstShow.value) {
        ...
        kvList.forEach((c: any) => {
          moduleTableTotalData.forEach((item: any) => {
            const findItem = customerTargetPrice.value.find(
              (ct: any) => ct.product === item.product && ct.kv === c.gradientValue
            )
            if (findItem) {
              arr.push({
                ...findItem,
...
const init = async (tempAuditFlowId?: any) => {
  ...
  if (tempAuditFlowId || query.auditFlowId) {
    ...
    if (viewDataRes.result) {
      ...
      if (viewDataRes.result.opinion && !tempAuditFlowId) {
        state.opinion = viewDataRes.result.opinion
      }
      if (viewDataRes.result.quickQuoteAuditFlowId && !tempAuditFlowId) {
